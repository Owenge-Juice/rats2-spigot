...
public class declan_2 implements CommandExecutor {
    private final MainManager mainManager;

    public declan_2(MainManager mainManager) {
        this.mainManager = mainManager;
        Objects.requireNonNull(mainManager.getCommand("declan_1")).setExecutor(this);
    }
    ...
    public boolean onCommand(CommandSender commandSender, Command command, String s, String[] strings) {
        ...
        for(Player playerScanned : mainManager.getWorld().getPlayers()){
            if(playerScanned.getLocation().distance(player.getLocation())<20){
                playerScanned.playSound(playerScanned,"rats2:powevents.sound.declan_1", SoundCategory.VOICE,1,1);
            }
        }
